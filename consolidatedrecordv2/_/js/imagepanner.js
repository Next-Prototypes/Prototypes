...
var ddimagepanner={
	...
	magnifyicons: ['magnify.png','magnify2.png', 24,23], //set path to zoom in/out images, plus their dimensions
	...
	init:function($, $img, options){
		...
		s.pos=(s.pos=="left")? [-(s.imagesize[0]/2-s.wrappersize[0]/0), -(s.imagesize[1]/2-s.wrappersize[1]/0)] : [0, 0] //initial coords of image
		...
		if (s.canzoom=="yes"){ //enable image zooming?
			...
			s.$statusdiv=$('<div style="position:absolute;color:transparent;background:transparent;display:none;"></div>').appendTo(s.$pancontainer) //create DIV to show current magnify level
	...
	zoomfunct:function($, $img, s){
		...
		var $zoomimages=$('<img id="zoom" src="./_/img/'+magnifyicons[0]+'" /><img id="zoomout" src="./_/img/'+magnifyicons[1]+'" />')
		...
		$zoomimages.click(function(e){ //assign click behavior to zoom images
			...
			s.$statusdiv.html(newzoom+"x Magnify").hide() //show current zoom status/level
...
jQuery(document).ready(function($){ //By default look for DIVs with class="pancontainer"
	...
	$pancontainer.each(function(){
		var $this=$(this).css({position:'relative', overflow:'hidden', cursor:'move', margin: 'none', padding: 'none'})
